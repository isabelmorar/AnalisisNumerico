...
def spline_page(request):
    if request.method == 'POST':
        x = request.POST.get('x')
        y = request.POST.get('y')
        d = int(request.POST.get('tipo_de_spline')) # 1: lineal, 2: cuadratico, 3: cubico
        df=0
        mensaje=""
        tipo=""
        ...
        # Quadratic
            ...
            try:
                ...
                bad= True
                mensaje = "La función no puede ser solucionada con este algoritmo. \n"
            if np.linalg.det(A)==0 or bad: 
                flag=True
                mensaje = mensaje + "La matriz obtenida no es invertible, por lo tanto se presenta la siguiente aproximación del polinomio por medio de la implementación de la librería interp1d de spline"
